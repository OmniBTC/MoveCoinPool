...
module coin_pool::pool_test {
    ...
    fun test_set_relayer(owner: &signer, user: &signer) {
        ...
        set_relayer<AptosCoin>(owner, user_addr);

        check_relayer<AptosCoin>(user);
    }
    ...
    fun test_supply(
        ...
        supply<AptosCoin>(user, user_supply_amount);
        assert!(coin::balance<AptosCoin>(user_addr) == user_init_amount - user_supply_amount, 2);

        assert!(pool_coins<AptosCoin>() == user_supply_amount, 3);
    }
    ...
    fun test_withdraw(
        ...
        set_relayer<AptosCoin>(owner, relayer_addr);

        assert!(coin::balance<AptosCoin>(user_addr) == user_init_amount - user_supply_amount, 1);
        assert!(pool_coins<AptosCoin>() == user_supply_amount, 2);
        withdraw<AptosCoin>(relayer, user_addr, user_withdraw_amount, 0);
        assert!(coin::balance<AptosCoin>(user_addr) == user_init_amount, 3);
        assert!(pool_coins<AptosCoin>() == 0, 4);
    }
    ...
    fun test_withdraw_nonce(
        ...
        let user_addr = signer::address_of(user);
        let user_withdraw_amount = 50;
        supply<AptosCoin>(user, user_withdraw_amount);
        withdraw<AptosCoin>(relayer, user_addr, user_withdraw_amount, nonce);
    }
    ...
    fun test_borrow(
        ...
        assert!(pool_coins<AptosCoin>() == user_init_amount, 1);
        coin::register_for_test<AptosCoin>(borrow_user);
        assert!(coin::balance<AptosCoin>(borrow_user_addr) == 0, 2);

        borrow<AptosCoin>(relayer, borrow_user_addr, user_borrow_amount, 0);
        assert!(pool_coins<AptosCoin>() == user_init_amount - user_borrow_amount, 3);
    ...
    fun test_repay(
        ...
        let user_addr = signer::address_of(user);
        assert!(pool_coins<AptosCoin>() == 0, 1);
        assert!(coin::balance<AptosCoin>(user_addr) == user_init_amount, 2);
        repay<AptosCoin>(user, user_repay_amount);
        assert!(pool_coins<AptosCoin>() == user_repay_amount, 3);
